...
export const getAllCountries = () => {
  return dispatch => {
    axios
      .get("https://api.worldbank.org/v2/country?format=json&per_page=10000")
      .then(response => {
        const allCountries = [];
        ...
        const result = response.data[1].filter(country =>
          givenCountriesFromReaktor.includes(country.name)
...
export const getAllYears = () => {
  return dispatch => {
    axios
      .get(
        `https://api.worldbank.org/v2/country/FIN/indicator/SP.POP.TOTL?format=json&per_page=100`
      )
      .then(response => {
        let years = [];
